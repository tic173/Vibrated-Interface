function [Ac,v,zeta_all,w1,w2,w1p,w2p] = Floquet_cylindrical_pinned(Ac_guess,omega_star, R0, m, C, Bd, At, eta, n, varargin)
% s_star - growth rate
...
%%
    ...
    lambda1 = 2*lm_star1/R0./(1+beta_star.^2.*lm_star1.^2/R0^2).*Im_prime1./Im1.*(beta_star.^2./(beta_star.^2-m^2));
     ...
     Sigma1 = 1*diag(lambda1(:))*(kron(ones(lb,1),eye(Ln)))* (kron(ones(lb,1),eye(Ln)))';

      Sigma2 = 1*diag(lambda1(1,:))*ones(lb,1)*ones(size(w));

 %      lambda1
 % 
 %     2*g_sign-(1+At)/At/1/Bd./lm_star1.^2;
 % 
 % sum(Aj_P,2)
 % 
 %       sum(Aj_P,2)-sum(Aj_P.*lambda1,2) + sum(Aj_H,2)


    I1 = eye(Ln);

     Jm = kron(ones(1,lb),I1)*(eye(lb*Ln)-Sigma1);

     Q = null(Jm);

    % Q=1;

    A0 = blkdiag(diag(Aj_P(:))*(eye(lb*Ln)-Sigma1)+diag(Aj_H(:)));
    B0 = kron(eye(lb),B1);
    ...
    D1       =  (diag(D));
    % 
    D       =  real(D1(abs(imag(D1))<tol*abs(real(D1))  & real(D1)>=0*max(a_guess) ) );
    V       =  V(:,(abs(imag(D1))<tol*abs(real(D1))  & real(D1)>=0*max(a_guess) ));


    [D,idx]       =  sort(D,'ascend','ComparisonMethod','real');
  
    Ac =  min(D(real(D)>=0*max(a_guess)));

    V = V(:,idx);

    v = (eye(lb*Ln)-Sigma1)*Q*V(:,1);
    ...
    zeta_all = reshape((Q*zeta_all_p).*v,Ln,[]);
...
for nj = Ln
    ...
    d3wdz3_2 =  (coeffs(5)-coeffs(6)+coeffs(7)*q2^3-coeffs(8)*q2^3)*beta_star^1;
     ...
     An_P(n_idx) =      2*dwdz*( (s_star+1i*nj*omega_star)/beta_star^2 ...
    + 3*C*(1-eta)*(1+At)/(2*At) ) ...
