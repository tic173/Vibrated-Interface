...
% Reuses stored lambda_j, g(j,k), and h(j,k); it does not rerun mode or
% forced solves.
...
output = saved.output;
assert_current_linear_operator(output);
adjointProjectionValidated = current_adjoint_projection(output);
...
coefficientModes = saved_modes(wnl);
modeSetValidity = wnl_validate_cubic_mode_set(coefficientModes);
coordinateTypes = cellfun(@wnl_mode_coordinate_type,coefficientModes, ...
    'UniformOutput',false);
requiredConvention = 'V2-real-self-conjugate-physical-peak';
savedConvention = '';
if isfield(wnl,'coefficientConventionVersion')
    savedConvention = char(wnl.coefficientConventionVersion);
end
if any(strcmp(coordinateTypes,'real')) && ...
        ~strcmp(savedConvention,requiredConvention)
    error('vi_saved_small_amplitude_transient:LegacyRealModeCoefficients', ...
        ['This cache contains an axisymmetric self-conjugate mode, but its ', ...
         'cubic coefficients predate the corrected real-coordinate ', ...
         'combinatorics. Re-run the main driver with ', ...
         'postprocessSavedCoefficientsOnly=false, modeRecoveryOnly=false, ', ...
         'and reuseRecoveredModes=true. The saved direct/adjoint modes can ', ...
         'be reused; only the nonlinear coefficients must be recomputed.']);
end
phaseSensitiveRequired = false;
for targetMode = 1:numel(coefficientModes)
    for sourceMode = 1:numel(coefficientModes)
        phaseSensitiveRequired = phaseSensitiveRequired || ...
            (targetMode ~= sourceMode && ...
            strcmp(coordinateTypes{targetMode},'complex') && ...
            strcmp(coordinateTypes{sourceMode},'complex') && ...
            wnl_phase_sensitive_coupling_allowed( ...
            coefficientModes{targetMode},coefficientModes{sourceMode}));
    end
end
if phaseSensitiveRequired && ~isfield(wnl,'phaseSensitiveG')
    error('vi_saved_small_amplitude_transient:LegacyPhaseCoupling', ...
        ['This cache contains complex modes whose symmetry permits ', ...
         'phase-sensitive cubic coupling, but it predates those ', ...
         'coefficients. Re-run the main driver with ', ...
         'postprocessSavedCoefficientsOnly=false, modeRecoveryOnly=false, ', ...
         'and reuseRecoveredModes=true. The direct/adjoint modes can be ', ...
         'reused; the nonlinear coefficients must be recomputed.']);
end
amplitudeScale = wnl_mode_amplitude_scale(coefficientModes);
if isfield(wnl,'amplitudeScalesToPeakZetaOverH')
    amplitudeScale = wnl.amplitudeScalesToPeakZetaOverH(:);
elseif isfield(wnl,'amplitudeScaleToPeakZetaOverH')
    amplitudeScale = wnl.amplitudeScaleToPeakZetaOverH(:);
end
if isfield(wnl,'gPhysicalPeak')
    gInZetaUnits = wnl.gPhysicalPeak;
else
    gInZetaUnits = wnl_physical_cubic_coefficients( ...
        g,coefficientModes);
end
phaseSensitiveGInZetaUnits = complex(zeros(size(gInZetaUnits)));
if isfield(wnl,'phaseSensitiveGPhysicalPeak')
    phaseSensitiveGInZetaUnits = ...
        wnl.phaseSensitiveGPhysicalPeak;
elseif isfield(wnl,'phaseSensitiveG')
    phaseSensitiveGInZetaUnits = ...
        wnl_physical_cubic_coefficients( ...
        wnl.phaseSensitiveG,coefficientModes);
end
savedA0 = output.initialConditions.complexAmplitudesOverH(:);
A0 = resolve_initial_amplitudes( ...
    settings,savedA0,numel(lambda),coordinateTypes);
...
transient = build_saved_transient(transientModel,timeRequested,A0, ...
    lambda,gInZetaUnits,limits,phaseSensitiveGInZetaUnits);
...
strictCoefficients = strictCoefficients && adjointProjectionValidated;
...
if ~strictCoefficients
    if ~adjointProjectionValidated
        warning('vi_saved_small_amplitude_transient:LegacyAdjointProjection', ...
            ['The saved cubic coefficients predate pressure-nullspace ', ...
             'bordering of the cylinder adjoint. They are displayed only ', ...
             'as exploratory values; rerun the main driver for reliable ', ...
             'coefficients.']);
    elseif exploratoryCoefficients
...
result.phaseSensitiveGInZetaUnits = ...
    phaseSensitiveGInZetaUnits;
result.gInternalCoordinates = g;
...
result.coefficientConventionVersion = savedConvention;
...
result.adjointProjectionValidated = adjointProjectionValidated;
...
if isfield(output,'operatorMetadata') && ...
    ...
    radialSettingNames = {'radialCorrectionPolicy', ...
        'radialCorrectionRelativeL2Threshold', ...
        'radialCorrectionTemporalSamples', ...
        'plotRadialModeCorrections'};
    for radialSettingIndex = 1:numel(radialSettingNames)
        radialSettingName = radialSettingNames{radialSettingIndex};
        if isfield(settings,radialSettingName)
            interfaceSettings.(radialSettingName) = ...
                settings.(radialSettingName);
        end
    end
    ...
    if numel(A0) >= 2 && ~isempty(sweepAmplitudes)
        result.secondModeInitialAmplitudeSweep = ...
            ...
            lambda,gInZetaUnits,phaseSensitiveGInZetaUnits,limits, ...
            output,wnl,amplitudeScale, ...
...
function assert_current_linear_operator(output)
supportedVersions = {'V2-pressure-compatible-sidewall', ...
    'V3-pressure-compatible-stress-free-sidewall'};
savedVersion = '';
if isfield(output,'operatorMetadata') && ...
        isstruct(output.operatorMetadata) && ...
        isfield(output.operatorMetadata,'linearOperatorVersion')
    savedVersion = char(output.operatorMetadata.linearOperatorVersion);
end
if any(strcmp(savedVersion,supportedVersions))
    return;
end
if isempty(savedVersion)
    savedVersion = 'legacy/unspecified';
end
error('vi_saved_small_amplitude_transient:LegacyLinearOperator', ...
    ['This coefficient cache uses linear operator %s; the current solver ', ...
     'requires a supported pressure-compatible sidewall operator. Its ', ...
     'saved full-state vectors may be used only as mode-', ...
     'recovery warm starts. Set postprocessSavedCoefficientsOnly=false ', ...
     'and run the main driver to recover the modes and recompute the ', ...
     'coefficients once.'],savedVersion);
end

function tf = current_adjoint_projection(output)
requiredVersion = 'V2-pressure-nullspace-bordered-temporal-schur';
tf = isfield(output,'operatorMetadata') && ...
    isstruct(output.operatorMetadata) && ...
    isfield(output.operatorMetadata,'adjointSolverVersion') && ...
    strcmp(char(output.operatorMetadata.adjointSolverVersion), ...
    requiredVersion);
end

function transient = build_saved_transient(transientModel,timeRequested,A0, ...
        lambda,g,limits,phaseSensitiveG)
switch transientModel
    case 'smallAmplitudeCorrection'
        transient = vi_cubic_transient_correction( ...
            timeRequested,A0,lambda,g,limits,phaseSensitiveG);
...
[linearTerms,nonlinearTerms,crossTerms,rateCoefficients,rateRatios] = ...
    landau_term_diagnostics(termState,lambda,g,phaseSensitiveG, ...
    limits.linearRateFloor);
...
transient.phaseSensitiveRhsTerms = conj(termState).* ...
    ((termState.^2)*phaseSensitiveG.');
...
function [linearTerms,nonlinearTerms,crossTerms,rateCoefficients,ratios] = ...
        landau_term_diagnostics(amplitudes,lambda,g,phaseSensitiveG, ...
        linearRateFloor)
...
phaseTerms = conj(amplitudes).*((amplitudes.^2)*phaseSensitiveG.');
nonlinearTerms = nonlinearTerms+phaseTerms;
rateCoefficients = nonlinearTerms./max(abs(amplitudes),1.0e-14);
ratios = abs(nonlinearTerms)./max(abs(linearTerms), ...
    linearRateFloor*max(abs(amplitudes),1.0e-14));
end
...
function sweep = build_second_mode_probe_sweep(amplitudes,timeRequested, ...
        periodsRequested,indices,A0,lambda,gInZetaUnits, ...
        phaseSensitiveGInZetaUnits,limits,output,wnl, ...
...
function modes = saved_modes(wnl)
if isfield(wnl,'modes')
    modes = wnl.modes(:);
elseif isfield(wnl,'mode')
    modes = {wnl.mode};
else
    error('vi_saved_small_amplitude_transient:MissingModes', ...
        'The saved coefficient result does not contain retained modes.');
end
end
...
function A0 = resolve_initial_amplitudes( ...
        settings,savedA0,numberOfModes,coordinateTypes)
...
for modeIndex = 1:numberOfModes
    ...
    imaginaryTolerance = 1.0e-10*max(1,abs(A0(modeIndex)));
    if abs(imag(A0(modeIndex))) > imaginaryTolerance
        error('vi_saved_small_amplitude_transient:RealModePhase', ...
            ['Mode %d is self-conjugate and has a real signed ', ...
             'amplitude. Its saved or overridden phase must be 0 or pi ', ...
             'modulo 2*pi.'],modeIndex);
    end
    A0(modeIndex) = real(A0(modeIndex));
end
end
...
mapping = { ...
    ...
    'plotSecondModeInitialAmplitudeSweep', ...
        'plotSecondModeInitialAmplitudeSweep'; ...
    'radialCorrectionPolicy','radialCorrectionPolicy'; ...
    'radialCorrectionRelativeL2Threshold', ...
        'radialCorrectionRelativeL2Threshold'; ...
    'radialCorrectionTemporalSamples', ...
        'radialCorrectionTemporalSamples'; ...
    'plotRadialModeCorrections','plotRadialModeCorrections'};
...
currentContactLine = boundary_setting(input,'contactLine','free');
savedContactLine = boundary_setting(savedInput,'contactLine','free');
currentSidewall = boundary_setting( ...
    input,'sidewallTangentialCondition','legacyFreeSlide');
savedSidewall = boundary_setting( ...
    savedInput,'sidewallTangentialCondition','legacyFreeSlide');
if ~strcmpi(currentContactLine,savedContactLine) || ...
        ~strcmpi(currentSidewall,savedSidewall)
    error('vi_saved_small_amplitude_transient:CaseMismatch', ...
        ['The current contact-line or sidewall tangential model does ', ...
         'not match the saved coefficient cache.']);
end
end

function value = boundary_setting(input,name,defaultValue)
value = defaultValue;
if isfield(input,'boundary') && isstruct(input.boundary) && ...
        isfield(input.boundary,name) && ~isempty(input.boundary.(name))
    value = char(input.boundary.(name));
end
end
