...
validateattributes(opts.forcedModelSolverOnly,{'logical','numeric'}, ...
    {'scalar'});
fallbackTolerance = forced_fallback_tolerance(opts);
forcedSolveWallClock = tic;
forcing = forcingCoeff(:);
matches = wnl_matching_modes(neutralModes, spec);
lazyModelOnly = isempty(matches) && opts.forcedModelSolverOnly && ...
    isfield(model,'solveForced') && ...
    isa(model.solveForced,'function_handle') && ...
    isfield(model,'slowBlock') && ...
    isa(model.slowBlock,'function_handle');
if lazyModelOnly
    n = spec.ndof*numel(spec.n);
    block = struct('A',[],'Bslow',model.slowBlock(spec), ...
        'solve',model.solveForced);
else
...
if isempty(matches)
    ...
    if hasModelSolve
        ...
        if opts.forcedModelSolverOnly
            if lazyModelOnly
                gmresDiagnostics = disabled_gmres_information();
            else
                gmresOpts = opts;
                gmresOpts.forcedUseBlockGmres = false;
                [q, gmresDiagnostics] = block_gmres_solution( ...
                    block.A, forcing, q, spec, gmresOpts);
            end
            rankDiagnostics = disabled_rank_information( ...
                ['global fallback disabled: retaining only the ', ...
                 'model-specific forced-field candidate']);
        else
            [q, gmresDiagnostics] = block_gmres_solution( ...
            ...
            if structuredModelSolve && ...
                ...
                if currentResidual > fallbackTolerance && ...
                    ...
                    [minimumNormCandidate,rankDiagnostics] = ...
                        ...
                        block.A,forcing,block.Bslow,opts,q);
        ...
        elseif currentResidual > fallbackTolerance
            gmresDiagnostics.minimumNormFallbackUsed = true;
    ...
    if hasModelSolve && opts.forcedModelSolverOnly
        if lazyModelOnly
            modelRelativeResidual = model_solve_numeric( ...
                modelSolveDiagnostics,'relativeResidual');
            modelAcceptanceResidual = model_solve_numeric_default( ...
                modelSolveDiagnostics,'acceptanceRelativeResidual', ...
                modelRelativeResidual);
            [solveDiagnostics,algebraicCompletion] = ...
                model_solver_only_diagnostics( ...
                [],forcing,q,modelAcceptanceResidual);
        else
            [solveDiagnostics,algebraicCompletion] = ...
                model_solver_only_diagnostics(block.A,forcing,q);
        end
    else
...
if lazyModelOnly
    ...
    modelAcceptanceResidual = model_solve_numeric_default( ...
        modelSolveDiagnostics,'acceptanceRelativeResidual', ...
        modelRelativeResidual);
    solution.unborderedResidual = ...
        modelRelativeResidual*max(norm(forcing),eps);
    solution.acceptanceUnborderedResidual = ...
        modelAcceptanceResidual*max(norm(forcing),eps);
else
    solution.unborderedResidual = norm(block.A * q - forcing);
    solution.acceptanceUnborderedResidual = solution.unborderedResidual;
end
...
if isempty(matches)
    solution.acceptanceEquationResidual = ...
        solution.acceptanceUnborderedResidual;
else
    solution.acceptanceEquationResidual = solution.equationResidual;
end
solution.acceptanceRelativeEquationResidual = ...
    solution.acceptanceEquationResidual/max(solution.forcingNorm,eps);
solution.relativeConstraintResidual = solution.constraintResidual / ...
    max(norm(block.Bslow*q), 1.0);
solution.valid = isfinite(solution.acceptanceRelativeEquationResidual) && ...
...
if lazyModelOnly
    solution.valid = solution.valid && ...
        ...
        model_solve_logical(modelSolveDiagnostics,'passedPhysicalGate');
end
...
solution.exploratoryUsable = ...
    isfinite(solution.acceptanceRelativeEquationResidual) && ...
    isfinite(solution.relativeConstraintResidual) && ...
    solution.acceptanceRelativeEquationResidual <= exploratoryTolerance && ...
    solution.relativeConstraintResidual <= ...
        opts.forcedSolveConstraintTolerance;
if lazyModelOnly
    solution.exploratoryUsable = solution.exploratoryUsable && ...
        model_solve_logical(modelSolveDiagnostics,'available') && ...
        model_solve_logical_default(modelSolveDiagnostics, ...
...
if opts.verbose
    ...
    if abs(solution.acceptanceRelativeEquationResidual- ...
            solution.relativeEquationResidual) > ...
            64*eps*max(solution.relativeEquationResidual,1)
        fprintf(', pressure-compatible %.3e', ...
            solution.acceptanceRelativeEquationResidual);
    end
    fprintf(['), constraint residual %.3e, ', ...
        'valid = %d, refinement flag/iterations/restarts = %d/%d/%d\n'], ...
        solution.constraintResidual, ...
    ...
    if modelSolveDiagnostics.attempted
        fprintf(['  model forced solver: %s; available/gate = %d/%d; ', ...
            ...
            model_solve_numeric(modelSolveDiagnostics, ...
                'relativeResidual'));
        if model_solve_logical(modelSolveDiagnostics, ...
                'pressureCompatibilityProjectionApplied')
            fprintf(['  certified pressure-nullspace quotient: ', ...
                'accepted residual %.3e, removed compatibility ', ...
                'residual %.3e (ceiling %.3e), harmonics %d\n'], ...
                modelSolveDiagnostics. ...
                    pressureCompatibleRelativeResidual, ...
                modelSolveDiagnostics. ...
                    pressureCompatibilityRelativeResidual, ...
                opts.forcedCylinderSchurPressureCompatibilityTolerance, ...
                modelSolveDiagnostics. ...
                    pressureCompatibilityAnalyzedHarmonics);
        end
        ...
        if isfield(modelSolveDiagnostics,'blockNullityRange')
            methods = modelSolveDiagnostics.blockFactorMethods;
            if iscell(methods)
                methodText = strjoin(methods,', ');
            else
                methodText = char(methods);
            end
            fprintf(['  temporal block nullity range [%g, %g]; ', ...
                'factor method(s): %s\n'], ...
                modelSolveDiagnostics.blockNullityRange(1), ...
                modelSolveDiagnostics.blockNullityRange(2),methodText);
        end
    ...
    if ~solution.valid && ~lazyModelOnly && ...
            isfield(model,'residualLayout')
...
function [diagnostics,completion] = ...
        model_solver_only_diagnostics(A,b,q,relativeResidual)
if nargin < 4
    relativeResidual = norm(b-A*q)/max(norm(b),eps);
end
raw = struct('flag',0,'totalIterations',0,'restarts',0, ...
    'relativeResidual',relativeResidual, ...
    'pressureSafeRefinement',false);
diagnostics = struct('flag',0,'totalIterations',0,'restarts',0, ...
    'relativeResidual',relativeResidual,'rawRefinement',raw, ...
    'correctionEquationResiduals',zeros(0,1), ...
    'acceptedStepLengths',zeros(0,1),'trialCompletionCount',0, ...
    'pressureSafeRestarts',0,'pressureSafeRefinement',false, ...
    'exploratoryEarlyStop',false);
completion = disabled_completion_information( ...
    ['global forced-field fallback and refinement disabled; ', ...
     'the model-specific physical residual gate is decisive']);
end

function information = disabled_gmres_information()
information = struct('attempted',false,'accepted',false, ...
    'improvedSeed',false,'passedPhysicalGate',false, ...
    'minimumNormFallbackUsed',false, ...
    'minimumNormFallbackResidual',NaN,'flag',NaN, ...
    'iterations',0,'relativeResidual',NaN, ...
...
try
    [scaledCorrection,flag,reportedResidual,iteration,chunkInformation] = ...
        chunked_lsqr(objectiveA,objectiveB, ...
        opts.forcedSolveRefinementTolerance, ...
        opts.forcedSolveMaxIterations,opts);
    ...
    information.stagnated = chunkInformation.stagnated;
    information.numberOfChunks = chunkInformation.numberOfChunks;
catch solverError
    information.errorIdentifier = solverError.identifier;
    information.errorMessage = solverError.message;
end
...
function value = model_solve_logical_default( ...
        information,fieldName,defaultValue)
if isfield(information,fieldName)
    value = logical(information.(fieldName));
else
    value = defaultValue;
end
end
...
function value = model_solve_numeric_default( ...
        information,fieldName,defaultValue)
value = model_solve_numeric(information,fieldName);
if ~isfinite(value)
    value = defaultValue;
end
end
...
function tolerance = forced_fallback_tolerance(opts)
% An explicitly requested exploratory ceiling is also a computational stop:
% once a model-specific solution reaches it, a global minimum-norm solve
% cannot change strict validity and need not be paid for.
tolerance = opts.forcedSolveResidualTolerance;
if ~opts.stopOnUnconvergedForcedSolve && ...
        ~isempty(opts.forcedExploratoryResidualTolerance)
    tolerance = forced_exploratory_tolerance(opts);
end
end
...
for attempt = 1:opts.forcedSolveMaxRestarts
    ...
    residual = b-A*x;
    [correction,flag,~,iteration,chunkInformation] = chunked_lsqr( ...
        A,residual,opts.forcedSolveRefinementTolerance, ...
        opts.forcedSolveMaxIterations,opts);
    x = x + correction;
    ...
    lastFlag = flag;
    restarts = attempt;
    if flag == 0 || ~improved || chunkInformation.stagnated
        break;
    end
end
x = bestX;
...
function [x,flag,relativeResidual,totalIterations,information] = ...
        chunked_lsqr(A,b,tolerance,maximumIterations,opts)
...
currentResidual = norm(b-A*x)/scale;
relativeResidual = currentResidual;
totalIterations = 0;
numberOfChunks = 0;
stagnantChunks = 0;
flag = double(currentResidual > tolerance);
while totalIterations < maximumIterations && ...
        currentResidual > tolerance
    chunkIterations = min(opts.forcedRefinementChunkIterations, ...
        maximumIterations-totalIterations);
    remainder = b-A*x;
    [increment,chunkFlag,~,iteration] = lsqr( ...
        A,remainder,tolerance,chunkIterations);
    usedIterations = forced_iteration_count(iteration);
    if usedIterations <= 0
        flag = chunkFlag;
        break;
    end
    candidate = x+increment;
    candidateResidual = norm(b-A*candidate)/scale;
    relativeImprovement = (currentResidual-candidateResidual) / ...
        max(currentResidual,eps);
    numberOfChunks = numberOfChunks+1;
    totalIterations = totalIterations+usedIterations;
    flag = chunkFlag;
    if isfinite(candidateResidual) && ...
    ...
    if chunkFlag == 0 || stagnantChunks >= ...
            opts.forcedRefinementStagnationChunks
        break;
    end
end
relativeResidual = currentResidual;
information = struct('stagnated',stagnantChunks >= ...
    opts.forcedRefinementStagnationChunks, ...
    'numberOfChunks',numberOfChunks, ...
    'stagnantChunks',stagnantChunks);
end
end
