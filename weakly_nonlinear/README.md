...
The current mathematical convention and its physical assumptions are recorded
in [`THEORY_AUDIT.md`](THEORY_AUDIT.md). The older v17.1 manuscript describes
only the common-neutral, complex-amplitude convention and is not authoritative
for the operating-point or axisymmetric real-mode paths.
...
- cross-interaction coefficients between supported mode symmetry classes;
- separate real-axisymmetric and complex-conjugate-pair combinatorics;
- phase-sensitive cubic coupling for compatible complex H/SH pairs;
...
exact operating-point exponents and all terms supported by the implemented
mode classes,

```text
dA_j/dt = lambda_j A_j + A_j sum_k g(j,k)|A_k|^2
          + conj(A_j) sum_k h(j,k) A_k^2.
```

The phase-sensitive matrix `h` is nonzero only when azimuthal and Floquet
selection rules permit it. The code enumerates all symmetry-allowed cubic
monomials and rejects a retained set when the compact `g/h` equation omits
one. This includes independent modes with identical `(m,s)` and 1:3
azimuthal resonances; supporting those cases requires a full cubic tensor.
...
When phase-sensitive coupling is symmetry allowed, the correction also
contains the exact Duhamel integral generated by
`h(j,k)*conj(A_L,j)*A_L,k^2`; see `THEORY_AUDIT.md` for the full formula.
...
system using the saved `lambda_j`, `g(j,k)`, and phase-sensitive `h(j,k)`.
The cache path is generated
...
probe displacements, and the actual linear and complete cubic right-hand-side
terms. In `smallAmplitudeCorrection` mode it also
returns both perturbative validity diagnostics.
Available second-order interface fields are reconstructed as instantaneous
slaved-manifold fields, including at the initial time. They are not the
startup response from a full fluid state initialized with only the primary
interface displacement; that problem requires a time-domain perturbation or
DNS calculation.
...
amplitude through a magnitude and phase:

```matlab
input.numberOfModes = 1;  % or 2
input.initialConditions.amplitudesOverH = [1.0e-3; 1.0e-6];
input.initialConditions.phases = [0.0; 0.0];
```

For a self-conjugate axisymmetric H/SH branch, the coordinate is real and the
phase must be zero or pi modulo `2*pi`; these values select its sign. Distinct
conjugate pairs retain a continuous complex phase.
...
the example above gives `A_1(0)=10^-3` and `A_2(0)=10^-6`. The driver reports
the exact `lambda_j`, wrapped
...
supported `g(j,k)` and `h(j,k)` matrices, and compares the reconstructed
interface with the exact linear result. The original
`forcedSolveResidualTolerance=1e-6` is not
...
For two retained modes in a supported symmetry pair, the complete
nonresonant system is integrated:
...
problems twice. It reuses each mode's self-generated second-order field, the
common `q_AB=q_BA` sum field, and a conjugate difference field when that
relation applies. For two complex modes this reduces the usual ten forced
solves to six; real and mixed pairs require fewer fields under their own
combinatorics. The saved
...
Consequently, a mode assigned exactly zero remains zero: every retained term
contains that mode or its conjugate. Use a small nonzero seed when you want to
observe competition between the two modes.
...
change the operating-point `lambda_j`, `g(j,k)`, or `h(j,k)`. They remain stored in
...
system, and reconstructs every primitive variable. Every reconstructed
candidate retains the original unequilibrated residual
`norm(A*q-f)/norm(f)` in its diagnostics.

For an axisymmetric equal-order block only, the discrete pressure space may
contain pressure-only right null vectors and redundant matching left
compatibility directions. With
`forcedCylinderSchurUsePressureCompatibilityProjection=true`, the solver
first certifies that the detected right nullspace has zero descriptor mass and
is supported only on algebraic columns. It then removes only the corresponding
left-null component from each materially nonzero residual harmonic. The
quotient residual must pass `forcedSolveResidualTolerance`, while the removed
component must independently remain below
`forcedCylinderSchurPressureCompatibilityTolerance`. Both values are printed
and saved. No projection is available to `m~=0`, an uncertified nullspace, or
a compatibility defect above the ceiling.
...
linear, WNL, and linear-minus-WNL values at that endpoint. The comparison
assumes `operators.P` is the derivative with respect to one unit of `a/g0`.
Cubic amplitudes use a mode-dependent normalization derived automatically.
If the complex carrier has unit peak interface displacement, the physical
peak scale is `S=2` for a distinct conjugate pair and `S=1` for a
self-conjugate real mode. Cubic matrices in physical peak-displacement units
divide column `k` by `S_k^2`.
...
For two complex modes, the reconstruction adds both primary fields, both
pairs of self-generated mean/second-harmonic fields, and the cross-generated
...
Self-conjugate real modes use the corresponding real polynomial expansion and
do not duplicate a conjugate mean field. Mixed real/complex pairs count their
stored conjugate cross fields once each.
...
The sidewall model is selectable. The user driver defaults to the cylindrical
...
while `sidewallTangentialCondition='legacyFreeSlide'` retains the earlier
`d_r(r*u_theta)=0` approximation. For `m>0`, the true stress-free condition
generally makes the radial and temporal parts of a Floquet mode nonseparable.
The recovered interface harmonic is therefore decomposed as

```text
zeta_n(r) = b_n J_m(beta r) + c_n(r),
integral_0^R r J_m(beta r)^* c_n(r) dr = 0.
```

`vi_radial_mode_correction` obtains `b_n` by the discrete cylindrical
least-squares projection of each recovered interface harmonic. It does not
solve an additional fluid problem. The full recovered mode is always used in
the WNL solvability calculation. The following settings only decide whether
`c_n(r)` is retained in interface reconstruction and plotting:

```matlab
input.comparison.radialCorrectionPolicy = 'auto'; % auto, always, or never
...
In `auto` mode, the correction is retained when its time-and-radius weighted
L2 norm divided by the full primary-carrier norm exceeds the threshold. The
same selected carrier is used for corresponding exact-linear and cubic-WNL
surface comparisons. Changing the sidewall model changes the linear operator
and therefore requires a matching recovery/coefficient cache.
...
the internal complex-pair normalization gives `mu=1` and `g=4*g0`. In
user-facing physical peak-amplitude units, `S=2`, so `g_peak=g0`.
...
mu = result.mu;
g = result.gPhysicalPeak;
h = result.phaseSensitiveGPhysicalPeak;
rhs = @(T,A) wnl_rhs_landau(T,A,mu,g,h);
```
...
```math
\frac{dA_j}{dT}
=\mu_jA_j+\sum_k\left[g_{jk}A_j|A_k|^2
+h_{jk}\bar A_jA_k^2\right].
```
...
### Targeted `g21` numerical verification

For the strict 30 Hz, `a/g=3`, `m0l6-m0l2` result, run

```matlab
addpath('/Users/tianyichu/Documents/Vibrated-Interface/weakly_nonlinear')
report = vi_verify_g21_numerics();
```

The verifier computes only matrix entry `(2,1)`. Its default gauge sweep
changes the interior pressure-gauge row at fixed physics; the saved midpoint
baseline is reused without a new solve. Set `settings.sweep='step'` for the
quadratic/cubic directional-step check or `settings.sweep='all'` for both.
Each new configuration requires only the two second-order forced solves for
`g21`, instead of repeating the complete coefficient matrix, and no large
result is saved.
The total coefficient should be insensitive to the gauge, and both sweeps
should approach a step-independent value. Significant gauge variation is a
discrete pressure/adjoint artifact, while significant step variation points
to finite-difference cancellation or truncation.

If a relocated-gauge adjoint narrowly misses the production mode gate, an
explicit verification-only ceiling can be supplied without changing the
saved cache or the production defaults:

```matlab
settings.coefficientModeResidualTolerance = 6e-7;
report = vi_verify_g21_numerics([],settings);
```

Each configuration reports separately whether it still passes the original
strict gate.
