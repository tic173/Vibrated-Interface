...
assert(abs(result.gPhysicalPeak-g0) < 1.0e-10, ...
    'The physical peak-amplitude coefficient conversion failed.');
assert(strcmp(result.coordinateType,'complex'));
...
fprintf('  g : computed %.12g, expected %.12g\n', ...
    result.g, expectedG);
fprintf('  g in physical peak-amplitude units: %.12g\n', ...
    result.gPhysicalPeak);
end
