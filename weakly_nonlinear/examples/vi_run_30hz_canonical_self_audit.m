...
for index = 1:numel(families)
    ...
    switch stage
        case 'recovery'
            saved = load(definition.templateFile,'output');
...
for familyIndex = 1:numel(familyNames)
    ...
    saved = load(definition.outputFile,'output');
    output = saved.output;
